...
* `batch` int64, explicit `size` supported, empty group → 0, genuine `-inf` max preserved
...
| after compat | fallback text **NONE**, `ascatter_reduce` absent from the profiler API trace |
